...
/**
 * This is the extension manager of the Interfaceerror extension.
 */
public class InterfaceerrorManager extends GeneratedInterfaceerrorManager {
    /**
     * Edit the local|project.properties to change logging behavior (properties 'log4j.*').
     */
    private static final Logger LOG = Logger.getLogger(InterfaceerrorManager.class.getName());
     ...
     */
    public static InterfaceerrorManager getInstance() {
        return (InterfaceerrorManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
                InterfaceerrorConstants.EXTENSIONNAME);
    }
     ...
     */
    public InterfaceerrorManager() // NOPMD
    {
        if (LOG.isDebugEnabled()) {
            LOG.debug("constructor of InterfaceerrorManager called.");
        }
    }
    ...
    public void init() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("init() of InterfaceerrorManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("destroy() of InterfaceerrorManager called, current tenant: " + getTenant().getTenantID());
        }
    }
