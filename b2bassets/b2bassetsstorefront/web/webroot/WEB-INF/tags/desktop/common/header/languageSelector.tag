...
<c:if test="${fn:length(languages) > 1}">
	...
	<form:form action="${setLanguageActionUrl}" method="post" id="lang-form">
		...
		<ycommerce:testId code="header_language_select">
			<select class="chosen-select" name="code" id="lang-selector">
				<c:forEach items="${languages}" var="lang">
					<c:choose>
						<c:when test="${lang.isocode == currentLanguage.isocode}">
							<option value="${lang.isocode}" selected="selected" lang="${lang.isocode}">${lang.nativeName}</option>
						</c:when>
						<c:otherwise>
							<option value="${lang.isocode}" lang="${lang.isocode}">${lang.nativeName}
								<%-- (${lang.name}) --%>
							</option>
						</c:otherwise>
					</c:choose>
