...
<!-- <div class="span-14 productImage"> -->
	...
	<!-- above div commented below modified by venkatesh-->
	<div class="more-images">
		<ul id="carousel_alternate" class="jcarousel-skin alt">
			<c:forEach items="${galleryImages}" var="container" varStatus="varStatus">
				<li>
<%-- 					<span class="thumb ${(varStatus.index==0)? "active":""}"> --%>
						<a href="#"><img src="${container.thumbnail.url}" data-primaryimagesrc="${container.product.url}" data-galleryposition="${varStatus.index}" alt="${container.thumbnail.altText}" title="${container.thumbnail.altText}" /></a>	
<!-- 					</span> -->
				</li>
			</c:forEach>
		 </ul>
