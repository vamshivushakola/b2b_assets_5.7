...
<c:choose>
	...
	<c:when test="${empty product.volumePrices}">
		<c:choose>
			<c:when test="${(not empty product.priceRange) and (product.priceRange.minPrice.value ne product.priceRange.maxPrice.value) and ((empty product.baseProduct) or (not empty isOrderForm and isOrderForm))}">
				<div class="price-box">
					<span class="big-price"> <format:price priceData="${product.priceRange.minPrice}" />
					</span><span class="separator-multiprice"> - </span><span class="big-price"> <format:price priceData="${product.priceRange.maxPrice}" />
					</span>
				</div>
			</c:when>
			<c:otherwise>
