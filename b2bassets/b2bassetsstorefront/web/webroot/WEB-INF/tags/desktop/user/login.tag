...
<div class="userLogin">
	...
	<div class="description"><h3><spring:theme code="login.description"/></h3></div>
	<div class="required right"><p><spring:theme code="form.required"/></p></div>
...
<div class="clear_fix">
	...
	<form:form action="${action}" method="post" commandName="loginForm">
		...
		<ul class="form-list">
			<li><formUtil:formInputBox icon="fa-user" idKey="j_username" labelKey="login.username" path="j_username" inputCSS="text account" mandatory="true" /></li>
			<li><formUtil:formPasswordBox icon="fa-lock" idKey="j_password" labelKey="login.password" path="j_password" inputCSS="text password" mandatory="true" /><a href="javascript:void(0)" data-url="<c:url value='/login/pw/request'/>" class="password-forgotten"><spring:theme code="login.link.forgottenPwd" /></a></li>
		</ul>
		<span style="display: block; clear: both;"> <ycommerce:testId code="login_Login_button">
