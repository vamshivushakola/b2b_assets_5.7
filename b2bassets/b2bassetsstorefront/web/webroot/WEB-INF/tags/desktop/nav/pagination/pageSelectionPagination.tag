...
<ol class="pager">
	...
	<li><c:if test="${hasPreviousPage}">
			...
			<ycommerce:testId code="searchResults_previousPage_link">
				<a class="prev" href="${previousPageUrl}" rel="prev"></a>
			</ycommerce:testId>
		</c:if> <c:if test="${not hasPreviousPage}">
			<a href="#" class="hidden" onclick="return false"> <spring:theme code="${themeMsgKey}.linkPreviousPage" />
			</a>
		</c:if></li>
	<li><c:set var="limit" value="${numberPagesShown}" /> <c:set var="halfLimit">
			<fmt:formatNumber value="${limit/2}" maxFractionDigits="0" />
		</c:set> <c:set var="beginPage">
			<c:choose>
		...
		</c:set> <c:forEach begin="${beginPage}" end="${endPage}" var="pageNumber">
			<c:choose>
	...
	<li><c:if test="${hasNextPage}">
			<spring:url value="${searchUrl}" var="nextPageUrl" htmlEscape="true">
				<spring:param name="page" value="${searchPageData.pagination.currentPage + 1}" />
			</spring:url>
			<ycommerce:testId code="searchResults_nextPage_link">
				<a class="next" href="${nextPageUrl}" rel="next">
				</a>
			</ycommerce:testId>
		</c:if> <c:if test="${not hasNextPage}">
			<a href="#" class="hidden" onclick="return false"> <spring:theme code="${themeMsgKey}.linkNextPage" />
			</a>
		</c:if></li>
