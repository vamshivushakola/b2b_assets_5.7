...
<c:if test="${empty addressFormEnabled or addressFormEnabled}">
	<form:form method="post" commandName="addressForm" id="selectAddressForm">
		...
		<%-- <div id="countrySelector" data-address-code="${addressData.id}" data-country-iso-code="${addressData.country.isocode}" class="clearfix">
			 <formUtil:formSelectBox idKey="address.country"
		 ...
		 <!-- commented\added by kinnari -->
		 <ul class="form-list" id="countrySelector" data-address-code="${addressData.id}" data-country-iso-code="${addressData.country.isocode}" >
			<li> <formUtil:formSelectBox icon="fa-building" idKey="address.country"
			                           labelKey="address.country"
			                           path="countryIso"
			                           mandatory="true"
			                           skipBlank="false"
			                           ...
			                           itemValue="isocode"
			                           selectedValue="${addressForm.countryIso}"
			                           selectCSSClass="chosen-select-responsive select" /> </li> 
		 </ul>  
			...
			<c:if test="${not noAddress}">
				<ycommerce:testId code="multicheckout_cancel_button">
					<c:url value="${cancelUrl}" var="cancel"/>
					 ...
					 <a href="${cancel}" class="positive cancel"><spring:theme code="checkout.multi.cancel" text="Cancel"/></a> 
				</ycommerce:testId>
			</c:if>
			...
			<c:choose>
				...
				<c:otherwise>
					<ycommerce:testId code="multicheckout_saveAddress_button">
						...
						</button> --%><!-- commented by kinnari - removed 'right' from class -->
						<button class="positive right change_address_button show_processing_message" type="submit">
							<spring:theme code="checkout.multi.deliveryAddress.continue" text="Continue"/>
						</button>
					</ycommerce:testId>
