...
<%@ taglib prefix="common" tagdir="/WEB-INF/tags/desktop/common"%>
<%@ taglib prefix="breadcrumb" tagdir="/WEB-INF/tags/desktop/nav/breadcrumb"%>
<%@ attribute name="b2BUserGroupForm" required="true" type="de.hybris.platform.commerceorgaddon.forms.B2BUserGroupForm" %>
...
<p>
	<spring:theme code="text.company.manageUsergroup.editForm" text="Please use this form to update usergroup details" />
</p>

<p class="required">
	<spring:theme code="form.required" text="Fields marked * are required" />
</p>
<form:form action="${formUrl}" method="post" commandName="b2BUserGroupForm" id="createb2BUserGroupForm">
	<form:input type="hidden" name="originalUid" path="originalUid" id="originalUid" />
	<ul class="form-list">
		<li><formUtil:formInputBox idKey="b2busergroup.id" labelKey="b2busergroup.id" path="uid" inputCSS="text" mandatory="true" /></li>
		<li><formUtil:formInputBox idKey="b2busergroup.name" labelKey="b2busergroup.name" path="name" inputCSS="text" mandatory="true" /></li>
		<li><formUtil:formSelectBox selectCSSClass="chosen-select-responsive select" idKey="b2busergroup.parent" labelKey="b2busergroup.parent" path="parentUnit" mandatory="false" skipBlank="false" skipBlankMessageKey="form.select.empty" items="${branchSelectOptions}" /></li>
	</ul>
	<a href="${cancelUrl}" class="positive form no-confirm"><spring:theme code="b2busergroup.cancel" text="Cancel" /></a>
	<ycommerce:testId code="Unit_SaveChange_button">
