...
<table class="data-table quote-status-details-table">
	...
	<tbody>
		<c:forEach items="${users}" var="user">
			...
			<spring:url value="/my-company/organization-management/manage-units/approvers/confirm/remove" var="removeUserUrl">
				<spring:param name="unit" value="${unit.uid}" />
				<spring:param name="user" value="${user.uid}" />
				<spring:param name="role" value="${role}" />
			</spring:url>
			<tr>
				<td><span class="hidden-lg hidden-md hidden-sm mobile-label"><spring:theme code="text.company.manage.units.user.name" text="Name" /> : </span> <a href="${viewUserUrl}">${fn:escapeXml(user.firstName)}&nbsp;${fn:escapeXml(user.lastName)}</a></td>
				<td><span class="hidden-lg hidden-md hidden-sm mobile-label"><spring:theme code="text.company.manage.units.user.email" text="Email" /> : </span> ${fn:escapeXml(user.uid)}</td>
				<td><span class="hidden-lg hidden-md hidden-sm mobile-label"><spring:theme code="text.company.manage.units.user.roles" text="Roles" /> : </span> <c:forEach items="${user.roles}" var="group">
						<p>
				...
				<td class="last">
					<ul class="updates clear_fix">
						<li><a title="<spring:theme code="text.company.manage.unit.childrenUnits.edit" text="Edit" />" class="edit-address" href="${editUserUrl}"></a></li>
