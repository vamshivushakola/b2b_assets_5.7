...
<p>
	<spring:theme code="text.company.manage.units.editForm" text="Please use this form to update bussiness unit details" />
</p>

<p class="required">
	<spring:theme code="form.required" text="Fields marked * are required" />
</p>
<form:form action="${formUrl}" method="post" commandName="b2BUnitForm" id="b2bUnitform">
	<ul class="form-list">
		<form:input type="hidden" name="originalUid" path="originalUid" id="originalUid" />
		<li><formUtil:formInputBox idKey="b2bunit.id" labelKey="b2bunit.id" path="uid" inputCSS="text" mandatory="true" /></li>
		<li><formUtil:formInputBox idKey="b2bunit.name" labelKey="b2bunit.name" path="name" inputCSS="text" mandatory="true" /></li>
		<li><formUtil:formSelectBox selectCSSClass="chosen-select-responsive select" idKey="b2bunit.parent" labelKey="b2bunit.parent" path="parentUnit" mandatory="false" skipBlank="false" skipBlankMessageKey="form.select.empty" items="${branchSelectOptions}" /></li>
		<li><formUtil:formSelectBox selectCSSClass="chosen-select-responsive select" idKey="b2bunit.approvalProcess" labelKey="b2bunit.approvalProcess" path="approvalProcessCode" mandatory="false" skipBlank="false" skipBlankMessageKey="form.select.empty" items="${businessProcesses}" /></li>
	</ul>
	<ycommerce:testId code="Unit_Cancel_button">
		<a href="${cancelUrl}" class="positive form no-confirm"><spring:theme code="b2bunit.cancel" text="Cancel" /></a>
	</ycommerce:testId>
