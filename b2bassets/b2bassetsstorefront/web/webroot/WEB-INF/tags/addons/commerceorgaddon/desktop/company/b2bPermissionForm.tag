...
<%@ attribute name="saveUrl" required="true" type="java.lang.String"%>
<%@ attribute name="cancelUrl" required="true" type="java.lang.String"%>
<%-- <%@ attribute name="b2BPermissionForm" required="true" type="org.responsive.storefront.forms.B2BPermissionForm"%> --%>
...
<form:form action="${saveUrl}" id="b2BPermissionForm" commandName="b2BPermissionForm" method="POST">
	...
	<p>
		<spring:theme code="text.company.managePermissions.type.label" text="Permission type" />
		: ${b2BPermissionForm.permissionType}
	</p>
	<ul class="form-list">
		<li><formUtil:formInputBox idKey="text.company.managePermissions.name.label" labelKey="text.company.managePermissions.name.label" path="code" inputCSS="text" /></li>
		<li><formUtil:formSelectBox selectCSSClass="chosen-select-responsive select" idKey="text.company.managePermissions.unit.label" skipBlankMessageKey="text.company.costCenter.selectBox.unit" labelKey="text.company.managePermissions.unit.label" path="parentUnitName" mandatory="true" items="${b2bUnits}" /></li>
		<c:if test="${b2BPermissionForm.b2BPermissionTypeData.code ne 'B2BBudgetExceededPermission' }">
			<c:if test="${b2BPermissionForm.b2BPermissionTypeData.code eq 'B2BOrderThresholdTimespanPermission'}">
				<li><formUtil:formSelectBox selectCSSClass="chosen-select-responsive select" idKey="text.company.managePermissions.timespan.label" skipBlankMessageKey="text.company.managePermissions.selectBox.timespan" labelKey="text.company.managePermissions.timespan.label" path="timeSpan" mandatory="true" items="${b2bPeriodRanges}" /></li>
			</c:if>
			<li><formUtil:formSelectBox selectCSSClass="chosen-select-responsive select" idKey="text.company.managePermissions.currency.label" skipBlankMessageKey="text.company.costCenter.selectBox.currency" labelKey="text.company.managePermissions.currency.label" path="currency" mandatory="true" items="${b2bCostCenterCurrencies}" /></li>
			<li><formUtil:formInputBox idKey="text.company.managePermissions.value.label" labelKey="text.company.managePermissions.value.label" path="value" inputCSS="text" mandatory="true" /></li>
		</c:if>
	</ul>
	<ycommerce:testId code="Permission_Cancel_button">
		<a href="${cancelUrl}" class="positive form no-confirm"><spring:theme code="text.company.managePermissions.edit.cancelButton" text="Cancel" /></a>
	</ycommerce:testId>
	<ycommerce:testId code="Permission_Save_button">
		<button type="submit" class="positive form confirm">
			<spring:theme code="text.company.managePermissions.edit.saveButton" text="Save" />
		</button>
	</ycommerce:testId>
</form:form>
