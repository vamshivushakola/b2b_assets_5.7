...
<form:form action="${saveUrl}" id="editB2bBudgetform" commandName="b2BBudgetForm" method="POST">
	<p class="required">
	...
	<ul class="form-list">
		<li><formUtil:formInputBox idKey="text.company.budget.budgetId" labelKey="text.company.budget.budgetId" path="code" inputCSS="text" mandatory="true" /></li>
		<li><formUtil:formInputBox idKey="text.company.budget.name" labelKey="text.company.budget.name" path="name" inputCSS="text" mandatory="true" /></li>
		<li><formUtil:formSelectBox selectCSSClass="chosen-select-responsive select" idKey="text.company.budget.unit" skipBlankMessageKey="text.company.costCenter.selectBox.unit" labelKey="text.company.costCenter.unit.title" path="parentB2BUnit" mandatory="true" items="${b2bUnits}" /></li>
		<li><formUtil:formInputBox idKey="budgetStartDate" labelKey="text.company.budget.startDate" path="startDate" inputCSS="date text" mandatory="true" /></li>
		<li><formUtil:formInputBox idKey="budgetEndDate" labelKey="text.company.budget.endDate" path="endDate" inputCSS="date text" mandatory="true" /></li>
		<li><formUtil:formSelectBox selectCSSClass="chosen-select-responsive select" idKey="text.company.budget.currency" skipBlankMessageKey="text.company.costCenter.selectBox.currency" labelKey="text.company.costCenter.currency.title" path="currency" mandatory="true" items="${b2bCostCenterCurrencies}" /></li>
		<li><formUtil:formInputBox idKey="text.company.budget.amount" labelKey="text.company.budget.amount" path="budget" inputCSS="text" mandatory="true" /></li>
	</ul>
	<ycommerce:testId code="Budget_Cancel_button">
		<a href="${cancelUrl}" class="form no-confirm positive"><spring:theme code="text.company.budget.cancelButton.displayName" text="Cancel" /></a>
	</ycommerce:testId>
	<ycommerce:testId code="Budget_Save_button">
		<button type="submit" class="form confirm positive" id="SubmitBudget_button">
			<spring:theme code="text.company.budget.saveButton.displayName" text="Save" />
		</button>
	</ycommerce:testId>
</form:form>
