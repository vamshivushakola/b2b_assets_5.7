						...
						<c:if test="${numberShowing > 0 and fn:length(products) > numberShowing}">
							<p class="legend">
								...
								<c:if test="${fn:length(products) > numberShowing}">
									<a href="${cartUrl}"><spring:theme code="popup.cart.show.all"/></a>
								</c:if>
							</p>
						</c:if>
<%-- comment by namrata						end value should be end="${numberShowing - 1} --%>
						...
						<div class="cart_modal_popup">
							...
							<div class="prod_info">
								...
								<p class="prod_quantity"><spring:theme code="popup.cart.quantity.added"/>&nbsp;${quantityAdded}</p>
