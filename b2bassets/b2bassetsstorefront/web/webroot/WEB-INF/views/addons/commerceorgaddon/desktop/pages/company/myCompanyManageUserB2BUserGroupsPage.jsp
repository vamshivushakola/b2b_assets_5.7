...
<template:page pageTitle="${pageTitle}">
			...
			<div class="row">
				...
				<div class="col-lg-9">
					...
					<table id="order_history" class="data-table quote-status-details-table">
						...
						<tbody>
							<c:forEach items="${searchPageData.results}" var="result">
								<spring:url value="/my-company/organization-management/manage-usergroups/details/" var="viewUserGroupUrl">
									<spring:param name="usergroup" value="${result.uid}" />
								</spring:url>
								...
								<tr class="${result.selected ? 'selected' : ''}" id="row-${ycommerce:normalizedCode(result.uid)}">
									...
									<td><ycommerce:testId code="${action}_b2bunit_label">
											...
											<span> <a href="${unitUrl}">${result.unit.name}</a>
											</span>
										</ycommerce:testId></td>

									<td class="last"><ycommerce:testId code="${action}_actions_label">
											<ul class="updates clear_fix" id="${ycommerce:normalizedCode(result.uid)}">
	...
	{{if selected}}
		<li><span class="default-address"></span></li>
		<li><a class="remove-address deselectionLink" title="${deselectAction}" url="${actionLink}/deselect/?usergroup={{= id}}&user=${param.user}"></a></li>
	{{else}}
		<li><a class="default-address selectionLink" title="${selectAction}" url="${actionLink}/select/?usergroup={{= id}}&user=${param.user}"></a></li>
		<li><span class="remove-address"></span></li>
	{{/if}}
</script>
