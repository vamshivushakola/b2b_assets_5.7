		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-9">
					...
					<h2 class="subtitle">
						<spring:theme code="text.company.budget.viewPage.label" text="View Budget: {0}" arguments="${budget.name}" />
					</h2>

					<div class="last margin-bottom-10">
						<a href="${editBudgetDetailsUrl}" class="positive edit" id="EditBudget_button"><spring:theme code="text.company.costCenter.button.displayName" /></a>
						<c:choose>
							<c:when test="${budget.active}">
								<a href="${disableUrl}" class="positive disable" id="DisableBudget_button"><spring:theme code="text.company.budget.disableButton.displayName" /></a>
							</c:when>
							<c:otherwise>
								<a href="${enableUrl}" class="positive enable" id="EnableBudget_button"><spring:theme code="text.company.budget.enableButton.displayName" /></a>
							</c:otherwise>
						</c:choose>
					</div>
					<p>
					...
					<ul>
						<li><strong><spring:theme code="text.company.budget.budgetId" text="Budget ID" /> : </strong>${budget.code}</li>
						<li><strong><spring:theme code="text.company.budget.name" text="Budget Name" /> : </strong>${budget.name}</li>
						<li><strong><spring:theme code="text.company.budget.unit" text="Parent business unit" /> : </strong><a href="${viewBudgetUnitUrl}">${budget.unit.name}</a></li>
						<li><strong><spring:theme code="text.company.budget.startDate" text="Start Date" /> : </strong> <fmt:formatDate value="${budget.startDate}" /></li>
						<li><strong><spring:theme code="text.company.budget.endDate" text="End Date" /> : </strong> <fmt:formatDate value="${budget.endDate}" /></li>
						<li><strong><spring:theme code="text.company.budget.currency" text="Currency" /> : </strong>${budget.currency.isocode}</li>
						<li><strong><spring:theme code="text.company.budget.amount" text="Budget Amount" /> : </strong> <fmt:formatNumber value="${budget.budget}" /></li>
						<li><strong><spring:theme code="text.company.budget.enabledDisabled" text="Budget enabled/disabled" /> : </strong> <c:choose>
								<c:when test="${budget.active}">
									<spring:theme code="text.company.on" text="ON" />
								</c:when>
								<c:otherwise>
					...
					<c:choose>
						<c:when test="${not empty budget.costCenters}">
							<table class="data-table quote-status-details-table">
								<thead>
									<tr class="hidden-xs">
										<th><span><spring:theme code="text.company.costCenter.id" text="ID" /></span></th>
										<th><span><spring:theme code="text.company.costCenter.name" text="Name" /></span></th>
									</tr>
								...
								<tbody>
									<c:forEach items="${budget.costCenters}" var="costCenter">
										<tr>
											<td><spring:url value="/my-company/organization-management/manage-costcenters/view" var="viewCostCenterUrl">
													<spring:param name="costCenterCode" value="${costCenter.code}" />
												</spring:url> <span class="hidden-lg hidden-md hidden-sm mobile-label"> <spring:theme code="text.company.costCenter.id" text="ID" /> :
											</span> <a href="${viewCostCenterUrl}"> ${costCenter.code} </a></td>
											<td class="last"><span class="hidden-lg hidden-md hidden-sm mobile-label"> <spring:theme code="text.company.costCenter.name" text="Name" /> :
											</span> ${costCenter.name}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
