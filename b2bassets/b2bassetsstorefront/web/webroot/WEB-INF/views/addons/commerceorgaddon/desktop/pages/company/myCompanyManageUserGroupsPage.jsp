			...
			<div class="row">
				...
				<div class="col-lg-9">
					...
					<div class="last margin-bottom-10">
						<ycommerce:testId code="Usergroup_createNew_button">
							<a href="${createUserGroupUrl}" class="positive create"><spring:theme code="text.company.manageUsergroups.newUserGroupButton" text="Create New Usergroup" /></a>
						</ycommerce:testId>
					</div>
					<p>
						<spring:theme code="text.company.${action}.subtitle" />
					</p>
					<nav:pagination top="true" supportShowPaged="${isShowPageAllowed}" supportShowAll="${isShowAllAllowed}" searchPageData="${searchPageData}" searchUrl="/my-company/organization-management/manage-usergroups?sort=$
								{searchPageData.pagination.sort}" msgKey="text.company.${action}.page" numberPagesShown="${numberPagesShown}" />
					<table id="order_history" class="data-table quote-status-details-table">
						<thead>
							...
							<tr class="hidden-lg hidden-md hidden-sm">
								<th><spring:theme code="text.company.${action}.title" text="All UserGroups" /></th>
							</tr>
						</thead>
						<tbody>
							<c:forEach items="${searchPageData.results}" var="group">
								<tr id="row-${result.normalizedId}">
									<td><spring:url value="/my-company/organization-management/manage-usergroups/details/" var="viewUrl">
											<spring:param name="usergroup" value="${group.uid}" />
										</spring:url> <spring:url value="/my-company/organization-management/manage-units/details/" var="viewUnitUrl">
											<spring:param name="unit" value="${group.unit.uid}" />
										</spring:url> <ycommerce:testId code="${action}_uid_link">
											<span class="hidden-lg hidden-md hidden-sm mobile-label"> <spring:theme code="text.company.column.id.name" text="ID" /> :
											</span>
											<a href="${viewUrl}">${group.uid}</a>
										</ycommerce:testId></td>
									...
									<td><ycommerce:testId code="${action}_b2bunit_label">
											...
											<a href="${viewUnitUrl}">${group.unit.name}</a>
										</ycommerce:testId></td>
									<td class="last"><ycommerce:testId code="costCenter_status_label">
											...
											<span> <spring:theme code="text.company.status.active.${fn:length(group.members)>0}" />
											</span>
										</ycommerce:testId></td>
					...
					<nav:pagination top="false" supportShowPaged="${isShowPageAllowed}" supportShowAll="${isShowAllAllowed}" searchPageData="${searchPageData}" searchUrl="/my-company/organization-management/manage-usergroups?sort=$
								{searchPageData.pagination.sort}" msgKey="text.company.${action}.page" numberPagesShown="${numberPagesShown}" />
					<c:if test="${empty searchPageData.results}">
						<p>No entries.</p>
					</c:if>
