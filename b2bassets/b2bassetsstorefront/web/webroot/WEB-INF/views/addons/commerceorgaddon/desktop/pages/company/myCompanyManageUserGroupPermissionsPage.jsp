...
<template:page pageTitle="${pageTitle}">
			...
			<div class="row">
				...
				<div class="col-lg-9">
					...
					<table id="order_history" class="data-table quote-status-details-table">
						...
						<tbody>
							<c:forEach items="${searchPageData.results}" var="result">
								<spring:url value="/my-company/organization-management/manage-permissions/" var="viewUrl">
									<spring:param name="permission" value="${result.code}" />
								</spring:url>
								<tr class="${result.selected ? 'selected' : ''}" id="row-${ycommerce:normalizedCode(result.code)}">
	...
	{{if selected}}
		<li><span class="default-address"></span></li>
		<li><a class="remove-address deselectionLink" title="${deselectAction}" url="${actionLink}/deselect/?permission={{= id}}&usergroup=${param.usergroup}"></a></li>
	{{else}}
		<li><a class="default-address selectionLink" title="${selectAction}" url="${actionLink}/select/?permission={{= id}}&usergroup=${param.usergroup}"></a></li>
		<li><span class="remove-address"></span></li>
	{{/if}}
</script>
