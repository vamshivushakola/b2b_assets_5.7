	...
	bindProdTabs: function()
	{
		...
		if(typeof PRODUCTTABSARRAY == 'undefined' || PRODUCTTABSARRAY.length == 0) {
			...
			for (var index = PRODUCTTABSARRAY.length - 1; index >=0; index--) {
				$('#tab_01').after('<li>'+ PRODUCTTABSARRAY[index].componentTitle+'</li>');
				$(".resp-tabs-container div:first-child").after('<div class="'+ PRODUCTTABSARRAY[index].contentClass +'" id="tab-'+ PRODUCTTABSARRAY[index].componentUid+'">'+ PRODUCTTABSARRAY[index].componentContent+'</div>'); 
			}
		}
